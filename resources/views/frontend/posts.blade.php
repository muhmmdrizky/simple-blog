...
<div class="card mb-3">
  ...
  <div class="card-body text-center">
    <h3 class="card-title"><a class="text-decoration-none text-dark" href="/posts/{{ $posts[0]->slug }}">{{ $posts[0]->title }}</h3></a>

    <small>
      <p>
        By: <a href="/posts?author={{ $posts[0]->author->username }}"> {{ $posts[0]->author->name }} </a>
        Category: <a href="/posts?category={{ $posts[0]->category->slug ?? 'unknown'}}"> {{ $posts[0]->category->name }}</a>
...
<div class="container">
  <div class="row">
    ...
    <div class="col-md-4 mb-3">
      <div class="card">
        <div class="position-absolute p-2 bg-dark text-white opacity-75"><a class="text-decoration-none text-white" href="/posts?category={{ $post->category->slug }}"> {{ $post->category->name }} </a></div>
        @if ($post->image)
